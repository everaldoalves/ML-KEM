...
/*************************************************
* Name:        keccakx2_absorb_once (Introduzida por mim)
*
...
**************************************************/
static void keccakx2_absorb_once(uint64x2_t s[25],
                                 unsigned int r,
                                 const uint8_t *in0,
                                 const uint8_t *in1,
                                 size_t inlen,
                                 uint8_t p)
{
  ...
  // Inicializar o estado com zeros
  for(i = 0; i < 25; ++i)
    s[i] = vdupq_n_u64(0);

  // Loop principal de absorção
  while(inlen >= r) {
    for(i = 0; i < r / 8; ++i) {
      uint64_t val0 = *(const uint64_t *)(in0 + pos);
      uint64_t val1 = *(const uint64_t *)(in1 + pos);
      t = vcombine_u64(vcreate_u64(val0), vcreate_u64(val1));
      s[i] = veorq_u64(s[i], t);
      pos += 8;
    }
    inlen -= r;
    
    f1600x2(s, neon_KeccakF_RoundConstants);
  }

  // Processar o restante dos dados
  if(inlen > 0) {
    size_t partial_blocks = inlen / 8;
    for(i = 0; i < partial_blocks; ++i) {
      uint64_t val0 = *(const uint64_t *)(in0 + pos);
      uint64_t val1 = *(const uint64_t *)(in1 + pos);
      t = vcombine_u64(vcreate_u64(val0), vcreate_u64(val1));
      s[i] = veorq_u64(s[i], t);
    ...
    inlen -= 8 * partial_blocks;

    // Se ainda houver dados incompletos
    if(inlen > 0) {
      uint64_t val0 = 0, val1 = 0;
      memcpy(&val0, in0 + pos, inlen);
      memcpy(&val1, in1 + pos, inlen);
      t = vcombine_u64(vcreate_u64(val0), vcreate_u64(val1));

      uint64_t mask = (1ULL << (8 * inlen)) - 1;
      uint64x2_t mask_vec = vdupq_n_u64(mask);
      t = vandq_u64(t, mask_vec);

      s[i] = veorq_u64(s[i], t);
    }

    // Adicionar padding
    uint64_t pad = (uint64_t)p << (8 * (pos % 8));
    uint64x2_t pad_vec = vdupq_n_u64(pad);
    s[i] = veorq_u64(s[i], pad_vec);
  } else {
    // Adicionar padding quando inlen é múltiplo de r
    uint64_t pad = (uint64_t)p;
    uint64x2_t pad_vec = vdupq_n_u64(pad);
    s[0] = veorq_u64(s[0], pad_vec);
  }

  // Aplicar o bit final
  uint64_t final_mask = 1ULL << 63;
  uint64x2_t final_mask_vec = vdupq_n_u64(final_mask);
  s[(r / 8) - 1] = veorq_u64(s[(r / 8) - 1], final_mask_vec);
}
...
void shake128x2_absorb_once(keccakx2_state *state, const uint8_t *in0, const uint8_t *in1, size_t inlen) {    
    keccakx2_absorb_once(state->s, SHAKE128_RATE, in0, in1, inlen, 0x1F);
}
...
void shake128x2_init(keccakx2_state *state) {
    // Inicializa dois estados Keccak em paralelo, usando 25 "lanes" de 128 bits (SIMD)
    for (int i = 0; i < 25; i++) {
        state->s[i] = vdupq_n_u64(0);  // Inicializa todos os "lanes" de Keccak com zero
    }
}

static void keccakx2_finalize(v128 state[25], unsigned int rate, uint8_t delimited_suffix) {
    // Aplica o delimitador 0x1F diretamente no início do bloco
    ((uint8_t*)state)[0] ^= delimited_suffix;  // Aplicar delimitador no primeiro byte do bloco
    ((uint8_t*)state)[rate - 1] ^= 0x80;       // Aplica o padding de Keccak no último byte do bloco

    // Permutação final nos dois estados simultaneamente
    KeccakF1600_StatePermutex2(state);  // Executa a permutação nos dois estados
}



void shake128x2_finalize(keccakx2_state *state) {
    // Finaliza os dois estados Keccak aplicando o delimitador 0x1F no bloco atual
    keccakx2_finalize(state->s, SHAKE128_RATE, 0x1F);
}


void shake256x2_init(keccakx2_state *state) {
    // Inicializa dois estados Keccak (25 lanes de 128 bits) com zero
    for (int i = 0; i < 25; i++) {
        state->s[i] = vdupq_n_u64(0);  // Inicializa cada lane de 128 bits com zero
    }
}

void shake256x2_finalize(keccakx2_state *state) {
    // Aplica o delimitador 0x1F no início do bloco
    ((uint8_t*)state->s)[0] ^= 0x1F;  // Aplicar delimitador no primeiro byte

    // Aplica o padding 0x80 no último byte do bloco, com base em SHAKE256_RATE
    ((uint8_t*)state->s)[SHAKE256_RATE - 1] ^= 0x80;  // Aplicar padding

    // Aplica a permutação Keccak simultaneamente nos dois estados
